...
#define WALL_DISTANCE_TARGET 150  
#define WALL_DISTANCE_TOLERANCE 60  
#define WALL_FOLLOW_SPEED 200  
#define TURN_SPEED 150  

void setupQMC5883L() {
  Wire.beginTransmission(QMC5883_ADDRESS);
  Wire.write(0x0B);  // Control Register 2 - Reset
  ...
  Wire.write(0x1D);  // Continuous Mode, 200Hz, 8G range
  Wire.endTransmission();
}
...
void wallFollower() {
    // Read distance from VL53L0X
    int currentDistance = lox.readRangeContinuousMillimeters();
    
    if (lox.timeoutOccurred()) {
        Serial.println("VL53L0X Timeout - Stopping");
        stopMotors();
        return;
    }
    ...
    int error = currentDistance - WALL_DISTANCE_TARGET;

    if (currentDistance >= 150 ) {
        // We're at the right distance - move forward
        analogWrite(MOTOR_1_PIN_1, WALL_FOLLOW_SPEED);
        analogWrite(MOTOR_1_PIN_2, 0);
        analogWrite(MOTOR_2_PIN_1, WALL_FOLLOW_SPEED);
        analogWrite(MOTOR_2_PIN_2, 0);
        analogWrite(MOTOR_3_PIN_1, 0);
        analogWrite(MOTOR_3_PIN_2, 0);
    }
    else if (currentDistance >= 60) {
        // Too far from wall - turn slightly towards wall
        analogWrite(MOTOR_1_PIN_1, 0);
        analogWrite(MOTOR_1_PIN_2, 255);
        analogWrite(MOTOR_2_PIN_1, 255);
        analogWrite(MOTOR_2_PIN_2, 0);
        ...
        // Too close to wall - turn slightly away from wall
        analogWrite(MOTOR_1_PIN_1, 0);    // Right motor backward
        analogWrite(MOTOR_1_PIN_2, 255);
        analogWrite(MOTOR_2_PIN_1, 0);    // Left motor backward
        analogWrite(MOTOR_2_PIN_2, 255);
    ...
    // Add a small delay to prevent sensor reading too frequently
    delay(50);
}
